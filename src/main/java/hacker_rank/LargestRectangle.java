package hacker_rank;

import java.util.Stack;

public class LargestRectangle {

    static long largestRectangle(int[] h) {
        Stack<Integer> pStack = new Stack<>(), hStack = new Stack<>();
        long max = 0;

        for (int pos = 0; pos < h.length; pos++) {
            if (hStack.isEmpty() || h[pos] > hStack.peek()) {
                hStack.push(h[pos]);
                pStack.push(pos);
            } else if (h[pos] < hStack.peek()) {
                int pTemp = pStack.peek();
                while (!hStack.isEmpty() && h[pos] < hStack.peek()) {
                    pTemp = pStack.peek();
                    max = Math.max(max, calcMax(pStack, hStack, pos));
                }
                hStack.push(h[pos]);
                pStack.push(pTemp);
            }
        }
        while (!hStack.isEmpty()) {
            max = Math.max(max, calcMax(pStack, hStack, h.length));
        }
        return max;
    }

    static long calcMax(Stack<Integer> pStack, Stack<Integer> hStack, int pos) {
        int pTemp = pStack.pop();
        int hTemp = hStack.pop();
        return hTemp * (pos - pTemp);
    }

    public static void main(String[] args) {
//        System.out.println(largestRectangle(new int[]{11,11,10,10,10}));
//        System.out.println(largestRectangle(new int[]{8979,4570,6436,5083}));
//        System.out.println(largestRectangle(new int[]{8979,4570,6436,5083,7780,3269,5400,7579,2324,2116}));
//        System.out.println(largestRectangle(new int[]{6320, 6020, 6098, 1332, 7263, 672, 9472, 2838, 3401, 9494}));
        System.out.println(largestRectangle(new int[]{6873,7005,1372,5438,1323,9238,9184,2396,4605,162,7170,9421,4012,5302,6277,2438,4409,3391,4956,4488,622,9365,5088,6926,2691,6909,1050,2824,3538,5801,8468,411,9158,9841,2201,481,5431,1385,2877,36,1547,48,5809,1911,1702,8439,4349,6111,1830,5657,6951,8804,5022,8391,2083,7713,5300,3133,6890,5190,5286,1710,1953,4445,7903,4154,4926,3335,5539,4156,9723,3438,556,1885,5349,2258,324,6050,4722,8506,1707,1673,7310,3081,65,9393,7147,1717,8878,389,6908,4165,2099,5213,8610,3,9368,3536,9690,1259}));
    }
}
